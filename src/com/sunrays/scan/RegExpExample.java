...
/*		if ("Drive Mapping".equals(message)) {
			fileManager.appendRow(FileManager.DRIVE_MAPPING_SHEET, dirName,
					name, lineNumber, line);
		} else if ("IP Address".equals(message)) {
			fileManager.appendRow(FileManager.IP_ADDRESS_SHEET, dirName, name,
					lineNumber, line);
		} else if ("URLS".equals(message)) {
			fileManager.appendRow(FileManager.URL_SHEET, dirName, name,
					lineNumber, line);
		} else if ("Others".equals(message)) {
	...
	/*	File root = new File(rootDirectory);
		fileManager = new FileManager(rootDirectory, "scon.xls", messages);
		fileManager.open();
		scanDirectory(rootDirectory, root);
		fileManager.flush();*/
